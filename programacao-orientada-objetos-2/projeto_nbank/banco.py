...
class Conta:
    ...
    def transfere(self,destino,quantidade,senha = '123'):
        if senha == self._senha and quantidade > 0 and quantidade <= self.saldo:
            self.saldo -= quantidade
            destino.saldo += quantidade
            self._historico.transacao.append(f'Transferencia no valor de:{quantidade}R$ para conta Nº {destino.get_numero} - Data:{datetime.datetime.now()}\n')
            destino._historico.transacao.append(f'Recebimento de Transferencia no valor de:{quantidade}R$ da conta Nº {self.get_numero} - Data:{datetime.datetime.now()}\n')
            return True
        else:
            return False
...
class Historico:
    def __init__(self):
        self.data_abertura = datetime.datetime.today().strftime('%Y-%m-%d%H:%M')
        self.transacao = []
