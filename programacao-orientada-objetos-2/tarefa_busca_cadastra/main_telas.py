...
class Main(QMainWindow, Ui_Main):
    ...
    def botaoCadastra(self):
        ...
        if not(nome == '' or endereco == '' or cpf == '' or nascimento == ''):
            ...
            if(self.cad.cadastra(p)):
                ...
                QMessageBox.information(None, 'natan', 'Cadastro realizado com sucesso!')
                self.tela_cadastro.lineEdit.setText('')
                self.tela_cadastro.lineEdit_2.setText('')
                self.tela_cadastro.lineEdit_3.setText('')
                self.tela_cadastro.lineEdit_4.setText('')
            else:
                QMessageBox.information(None, 'natan', 'CPF informado já existe!')
        else:
            QMessageBox.information(None, 'natan', 'Todas campos devem ser preenchidos')
    ...
    def botaoBusca(self):
        ...
        cpf = self.tela_buscar.lineEdit_3.text()
        pessoa = self.cad.busca(cpf)

        if(pessoa != None):
            
            self.tela_buscar.lineEdit.setText(pessoa.nome)
            self.tela_buscar.lineEdit_2.setText(pessoa.endereco)
            self.tela_buscar.lineEdit_4.setText(pessoa.nascimento)
        else:
            QMessageBox.information(None,'natan', 'CPF não encontrado!')
